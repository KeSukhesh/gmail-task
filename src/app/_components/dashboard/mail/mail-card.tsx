  ...
  ({ mail, isSelected, onSelect }, ref) => {
    return (
      <button
        ref={ref}
        className={cn(
          "flex flex-col items-start gap-2 rounded-lg border p-3 text-left text-sm transition-all hover:bg-accent w-full min-w-0",
          isSelected && "bg-muted"
        )}
        onClick={onSelect}
      >
        <div className="flex items-center w-full min-w-0">
          ...
          >
            {/* Prefer internalDate for safeFormatDistance */}
            {safeFormatDistance(mail.internalDate || mail.date)}
          </div>
        </div>
        ...
        <div
          className="text-xs text-muted-foreground w-full break-words line-clamp-2"
        >
        ...
        {(mail.labels ?? []).length > 0 && (
          <div className="flex items-center gap-1 flex-wrap mt-1"> {/* Reduced gap and added mt-1 */}
            {(mail.labels ?? []).map((label) => (
              <Badge key={label} variant={getBadgeVariantFromLabel(label)} className="px-1.5 py-0.5 text-[10px]">
                {label}
              </Badge>
            ))}
          </div>
